...
class COMAFC(Model):
    ...
    def construct_value_net(self):
        ...
        if self.args.shared_parameters:
            ...
            for i in range(self.n_):
                value_dicts.append(nn.ModuleDict(
                    {'layer_1': nn.Linear((self.n_ + 1) * self.obs_dim + (self.n_ - 1) * self.act_dim, self.hid_dim), \
                     'layer_2': nn.Linear(self.hid_dim, self.hid_dim), \
    ...
    def value(self, obs, act):
        ...
        obs_own = obs.clone()
        obs = obs.unsqueeze(1).expand(batch_size, self.n_, self.n_,
                                      self.obs_dim)  # shape = (b, n, o) -> (b, 1, n, o) -> (b, n, n, o)
    ...
    def get_loss(self, batch):
        ...
        baselines = torch.sum(values * torch.softmax(action_out, dim=-1),
                              dim=-1)  # the only difference to ActorCritic is this  baseline (b,n)
