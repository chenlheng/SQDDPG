...
class SQDDPG(Model):
    ...
    def unpack_data(self, batch):
        ...
        actions = cuda_wrapper(torch.tensor(np.stack(list(zip(*batch.action))[0], axis=0), dtype=torch.float),
                               self.cuda_)
    ...
    def construct_value_net(self):
        ...
        if self.args.shared_parameters:
            ...
            for i in range(self.n_):
                value_dicts.append(
                    nn.ModuleDict({'layer_1': nn.Linear((self.obs_dim + self.act_dim) * self.n_, self.hid_dim), \
                                   'layer_2': nn.Linear(self.hid_dim, self.hid_dim), \
    ...
    def sample_grandcoalitions(self, batch_size):
        ...
        grand_coalitions = cuda_wrapper(
            torch.multinomial(torch.ones(batch_size * self.sample_size, self.n_) / self.n_, self.n_, replacement=False),
            self.cuda_)
        individual_map = cuda_wrapper(torch.zeros(batch_size * self.sample_size * self.n_, self.n_), self.cuda_)
        individual_map.scatter_(1, grand_coalitions.contiguous().view(-1, 1), 1)  # change grand_coal to onehot vec
        ...
        grand_coalitions = grand_coalitions.unsqueeze(1).\
            expand(batch_size * self.sample_size, self.n_, self.n_).contiguous().\
    ...
    def marginal_contribution(self, obs, act):
        ...
        obs = obs.unsqueeze(1).unsqueeze(2).expand(batch_size, self.sample_size, self.n_, self.n_,
                                                   self.obs_dim)  # shape = (b, n, o) -> (b, 1, n, o) -> (b, 1, 1, n, o) -> (b, n_s, n, n, o)
        obs = obs.contiguous().view(batch_size, self.sample_size, self.n_,
                                    self.n_ * self.obs_dim)  # shape = (b, n_s, n, n, o) -> (b, n_s, n, n*o)
        ...
        for i in range(self.n_):
            ...
            values.append(v)
        values = torch.stack(values, dim=2)  # [bs, n_s, n, 1]
        return values
    ...
    def get_loss(self, batch):
        ...
        shapley_values_sum = self.marginal_contribution(state, actions).mean(dim=1).contiguous().\
            view(-1, n).sum(dim=-1, keepdim=True).expand(batch_size, self.n_)
        # do the argmax action on the next value loss
        if self.args.target:  # True
        ...
        if self.args.normalize_advantages:  # False
