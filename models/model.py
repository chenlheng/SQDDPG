...
class Model(nn.Module):
    ...
    def update_target(self):
        for name, param in self.target_net.action_dicts.state_dict().items():
            update_params = (1 - self.args.target_lr) * param + self.args.target_lr * self.action_dicts.state_dict()[
                name]
        ...
        for name, param in self.target_net.value_dicts.state_dict().items():
            update_params = (1 - self.args.target_lr) * param + self.args.target_lr * self.value_dicts.state_dict()[
                name]
    ...
    def transition_update(self, trainer, trans, stat):
        if self.args.replay:
            ...
            if replay_cond:
                # print('training in %i' % trainer.steps)
