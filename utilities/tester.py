...
class PGTester(object):
    ...
    def action_logits(self, state, schedule, last_action, last_hidden, info):
        return self.behaviour_net.policy(state, schedule=schedule, last_act=last_action, last_hid=last_hidden,
                                         info=info)

    def run_step(self, state, schedule, last_action, last_hidden, info={}):
        state = cuda_wrapper(prep_obs(state).contiguous().view(1, self.args.agent_num, self.args.obs_size),
                             cuda=self.cuda_)
